...
class NewsAnalyzer:
    ...
    def _crawl_custom(self) -> Tuple[Dict, Dict, List]:
        """
        执行自定义数据源抓取（如 Twitter 趋势等第三方 API 数据源）

        输出格式与 _crawl_data() 完全一致：
            results: {source_id: {title: {ranks:[], url, mobileUrl}}}
            id_to_name: {source_id: display_name}
            failed_ids: [failed_source_id, ...]
        ...
        """
        if not self.ctx.twitter_enabled:
            return {}, {}, []

        twitter_config = self.ctx.custom_twitter_config

        # API Key 从环境变量读取（安全性）
        api_key = os.environ.get("TWITTER_API_KEY", "")
        if not api_key:
            print("[自定义数据源] Twitter: 未配置 API Key（设置环境变量 TWITTER_API_KEY）")
            return {}, {}, ["twitter-trends"]

        region = twitter_config.get("REGION", "worldwide")
        max_items = twitter_config.get("MAX_ITEMS", 50)

        print(f"[自定义数据源] Twitter: 开始抓取 (地区: {region}, 最多 {max_items} 条)...")
        ...
        try:
            ...
            if not results:
                print("[自定义数据源] Twitter: 未获取到数据")
                return {}, {}, [source_id]

            id_to_name = {source_id: source_name}
            failed_ids = []

            # 保存到存储后端（与 _crawl_data 相同的保存模式）
            crawl_time = self.ctx.format_time()
            crawl_date = self.ctx.format_date()
            news_data = convert_crawl_results_to_news_data(
                results, id_to_name, failed_ids, crawl_time, crawl_date
            ...
            if self.storage_manager.save_news_data(news_data):
                count = len(results.get(source_id, {}))
                print(f"[自定义数据源] Twitter: {count} 条趋势数据已保存到存储后端")

            return results, id_to_name, failed_ids

        except Exception as e:
            print(f"[自定义数据源] Twitter: 抓取失败 - {e}")
            if self.ctx.config.get("DEBUG", False):
                import traceback
                traceback.print_exc()
            return {}, {}, ["twitter-trends"]
    ...
    def run(self) -> None:
        ...
        try:
            ...
            # 抓取自定义数据源（如 Twitter 趋势），合并到热榜结果中
            custom_results, custom_id_to_name, custom_failed_ids = self._crawl_custom()
            if custom_results:
                results.update(custom_results)
                id_to_name.update(custom_id_to_name)
                failed_ids.extend(custom_failed_ids)
