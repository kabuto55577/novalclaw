...
def _load_custom_sources_config(config_data: Dict) -> Dict:
    """
    加载自定义数据源配置

    支持第三方 API 数据源（如 Twitter 趋势等）。
    每个数据源作为独立子配置项，通过 enabled 开关控制。
    """
    custom_sources = config_data.get("custom_sources", {})
    twitter = custom_sources.get("twitter", {})

    return {
        "TWITTER": {
            "ENABLED": twitter.get("enabled", False),
            "REGION": twitter.get("region", "worldwide"),
            "MAX_ITEMS": int(twitter.get("max_items", 50)),
        },
    }
...
def load_config(config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    # 自定义数据源配置（Twitter 趋势等）
    config["CUSTOM_SOURCES"] = _load_custom_sources_config(config_data)
