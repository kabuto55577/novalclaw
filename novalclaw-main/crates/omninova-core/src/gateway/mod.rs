...
use crate::skills::process_manager::ProcessRegistry;
...
use crate::tools::{
    ...
    TrendRadarBridge, WebFetchTool, WebSearchTool,
};
...
pub struct GatewayRuntime {
    ...
    /// Optional TrendRadar bridge for news intelligence tools.
    pub(crate) trendradar_bridge: Option<Arc<TrendRadarBridge>>,
    /// Process registry for managing external skill processes.
    pub(crate) process_registry: Arc<ProcessRegistry>,
}

impl GatewayRuntime {
    pub fn new(config: Config) -> Self {
        // TrendRadar bridge: always create; tools are discovered lazily.
        // Host/port configurable via env vars, defaulting to localhost:3333.
        let host = std::env::var("TRENDRADAR_MCP_HOST")
            .unwrap_or_else(|_| "127.0.0.1".to_string());
        let port: u16 = std::env::var("TRENDRADAR_MCP_PORT")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(3333);
        let trendradar_bridge = Some(Arc::new(TrendRadarBridge::new(&host, port)));
    ...
    pub async fn chat(&self, message: &str) -> anyhow::Result<String> {
        ...
        let tools = create_tools_for_route(
            ...
            &route_agent_name,
            self.memory.clone(),
            self.trendradar_bridge.as_deref(),
        )
        .await;
    ...
    pub async fn serve_http(mut self) -> anyhow::Result<()> {
        ...
        let app = Router::new()
            ...
            .route("/api/trendradar/health", get(http_api_trendradar_health))
            .route("/api/trendradar/tools", get(http_api_trendradar_tools))
            .route("/api/trendradar/call", post(http_api_trendradar_call))
            .route("/api/trendradar/refresh", post(http_api_trendradar_refresh))
            .route("/api/trendradar/process/start", post(http_api_trendradar_process_start))
            .route("/api/trendradar/process/stop", post(http_api_trendradar_process_stop))
            .route("/api/trendradar/process/status", get(http_api_trendradar_process_status))
...
async fn http_api_tools(
    ...
    let cfg = runtime.get_config().await;
    let tools = create_all_tools_async(
        &cfg,
        runtime.memory.clone(),
        runtime.trendradar_bridge.as_deref(),
    )
    .await;
...
// ─── TrendRadar API Handlers ────────────────────────────────────────────────

#[derive(Debug, serde::Deserialize)]
struct TrendRadarCallRequest {
    tool: String,
    #[serde(default)]
    arguments: serde_json::Value,
}

async fn http_api_trendradar_health(
    State(runtime): State<GatewayRuntime>,
) -> Result<Json<serde_json::Value>, Json<GatewayError>> {
    match &runtime.trendradar_bridge {
        Some(bridge) => {
            let healthy = bridge.is_healthy().await;
            Ok(Json(serde_json::json!({
                "available": true,
                "healthy": healthy,
                "detail": if healthy { "MCP server reachable" } else { "MCP server unreachable" }
            })))
        }
        None => Ok(Json(serde_json::json!({
            "available": false,
            "healthy": false,
            "detail": "TrendRadar not configured (set skills.trendradar.enabled=true in config)"
        }))),
    }
}

async fn http_api_trendradar_tools(
    State(runtime): State<GatewayRuntime>,
) -> Result<Json<serde_json::Value>, Json<GatewayError>> {
    match &runtime.trendradar_bridge {
        Some(bridge) => match bridge.discover_tools(false).await {
            Ok(tools) => {
                let tool_list: Vec<serde_json::Value> = tools
                    .iter()
                    .map(|t| {
                        serde_json::json!({
                            "name": format!("trendradar.{}", t.name),
                            "description": t.description,
        ...
        None => Err(Json(GatewayError {
            message: "TrendRadar bridge not configured".to_string(),
        })),
    }
}

async fn http_api_trendradar_call(
    State(runtime): State<GatewayRuntime>,
    Json(req): Json<TrendRadarCallRequest>,
) -> Result<Json<serde_json::Value>, Json<GatewayError>> {
    match &runtime.trendradar_bridge {
        Some(bridge) => {
            let tool_name = if req.tool.starts_with("trendradar_") {
                req.tool.strip_prefix("trendradar_").unwrap().to_string()
            } else {
                req.tool.clone()
            };
            match bridge.call_tool_direct(&tool_name, req.arguments).await {
...
async fn http_api_trendradar_refresh(
    State(runtime): State<GatewayRuntime>,
) -> Result<Json<serde_json::Value>, Json<GatewayError>> {
    match &runtime.trendradar_bridge {
        Some(bridge) => match bridge.discover_tools(true).await {
            Ok(tools) => {
                let count = tools.len();
                info!("TrendRadar tools refreshed: {count} tools");
                Ok(Json(serde_json::json!({
                    "ok": true,
                    "count": count,
                })))
            ...
            Err(e) => Err(Json(GatewayError {
                message: format!("Failed to refresh TrendRadar tools: {e}"),
            })),
        },
        None => Err(Json(GatewayError {
            message: "TrendRadar bridge not configured".to_string(),
...
async fn http_api_trendradar_process_start(
    State(runtime): State<GatewayRuntime>,
) -> Result<Json<serde_json::Value>, Json<GatewayError>> {
    let Some(proc) = runtime.process_registry.find("trendradar-mcp").await else {
        return Err(Json(GatewayError {
            message: "trendradar-mcp process not registered".to_string(),
        }));
    };

    match proc.start_and_wait(
    ...
    .await
    {
        Ok(()) => Ok(Json(serde_json::json!({
            "ok": true,
            "detail": "TrendRadar MCP server started and healthy",
        }))),
        Err(e) => Err(Json(GatewayError {
            message: e.to_string(),
        })),
...
async fn http_api_trendradar_process_stop(
    State(runtime): State<GatewayRuntime>,
) -> Result<Json<serde_json::Value>, Json<GatewayError>> {
    let Some(proc) = runtime.process_registry.find("trendradar-mcp").await else {
        return Err(Json(GatewayError {
...
async fn http_api_trendradar_process_status(
    State(runtime): State<GatewayRuntime>,
) -> Result<Json<serde_json::Value>, Json<GatewayError>> {
    let proc = runtime.process_registry.find("trendradar-mcp").await;
    let all_health = runtime.process_registry.health_all().await;

    // Compute trendradar_mcp health synchronously (avoid await in non-async closures).
    let trendradar_status = match &proc {
        Some(p) => {
            let health = p.health_status().await;
    ...
    Ok(Json(serde_json::json!({
        "trendradar_mcp": trendradar_status,
        "all_processes": all_health.iter().map(|(name, health)| {
            serde_json::json!({
                "name": name,
                "running": health.running,
                "healthy": health.healthy,
...
/// Create tools including any TrendRadar MCP tools that need async discovery.
///
/// TrendRadar tools are discovered lazily — if the MCP server is not running,
/// no tools are registered and a warning is logged instead of failing.
pub async fn create_all_tools_async(
    config: &Config,
    memory: Arc<dyn Memory>,
    trendradar_bridge: Option<&TrendRadarBridge>,
) -> Vec<Box<dyn Tool>> {
    let mut tools = create_all_tools(config, memory);

    if let Some(bridge) = trendradar_bridge {
        match bridge.discover_tools(false).await {
            Ok(defs) => {
                if defs.is_empty() {
                    debug!("TrendRadar MCP server reports 0 tools");
                } else {
                    match bridge.create_tools().await {
                        Ok(mcp_tools) => {
                            let count = mcp_tools.len();
                            tools.extend(mcp_tools);
                            info!("Loaded {count} TrendRadar MCP tools for Agent");
                        }
                        Err(e) => {
                            warn!("Failed to create TrendRadar tool wrappers: {e}");
                        }
                    }
                }
            }
            Err(e) => {
                debug!("TrendRadar MCP server not available, skipping tools: {e}");
            }
        }
    }
...
async fn create_tools_for_route(
    config: &Config,
    route_agent_name: &str,
    memory: Arc<dyn Memory>,
    trendradar_bridge: Option<&TrendRadarBridge>,
) -> Vec<Box<dyn Tool>> {
    let tools = create_all_tools_async(config, memory, trendradar_bridge).await;
...
mod tests {
    ...
    #[tokio::test]
    async fn delegate_allowed_tools_filter_default_toolset() {
        ...
        let tools = create_tools_for_route(&config, "researcher", memory, None).await;
